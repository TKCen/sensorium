(function () {
  ...
  const FLOW_DAG_COLUMN_ORDER = ["sensor", "emitter", "processor", "gate", "queue", "sink", "unknown", "candidate", "thread", "action", "outbox"];
  ...
  function buildFlowDagGraph(topoData, runtimeData) {
    const nodes = new Map();
    ...
    runtimeNodes.forEach(function (n) {
      if (n.origin === "instance" && !nodes.has(n.id)) {
        nodes.set(n.id, {
          id: n.id,
          kind: n.kind,
          label: compactId(n.id),
          origin: "instance",
          status: n.status,
    ...
    return {
      ...
      meta: {
        ...
        instance_node_count: runtimeNodes.filter(function (n) { return n.origin === "instance"; }).length,
        configured_edge_count: edges.length,
  ...
  function flowDagLayout(graph) {
    ...
    const width = Math.max(720, cols.length * 190);
    const colGap = cols.length > 1 ? (width - 160) / (cols.length - 1) : 0;
    const maxRows = Math.max.apply(null, cols.map(function (k) { return byKind[k].length; }).concat([1]));
    const height = Math.max(320, 78 + maxRows * 66);
    const positions = {};
    const positioned = [];
    cols.forEach(function (kind, ci) {
      const x = 32 + ci * colGap;
      (byKind[kind] || []).forEach(function (node, ri) {
        const y = 64 + ri * 66;
        const item = Object.assign({}, node, { x: x, y: y, col: kind });
  ...
  function FlowDagNode(props) {
    ...
    return h("button", {
      type: "button",
      className: "sx-flow-node" + (active ? " sx-flow-node-active" : "") + (node.origin === "topology" ? " sx-flow-node-topology" : " sx-flow-node-instance"),
      ...
      h("span", { className: "sx-flow-node-kind" }, node.kind),
      h("span", { className: "sx-flow-node-label" }, compactId(node.label)),
      h("span", { className: "sx-flow-node-state" }, node.status || "unknown", node.origin === "topology" && (node.configured_status === "disabled") ? " · disabled" : ""),
    );
  }
  ...
  function FlowDagCanvas(props) {
    ...
    return h("div", { className: "sx-flow-map-wrap" },
      h("div", { className: "sx-flow-map-canvas", style: { minWidth: layout.width + "px", height: layout.height + "px" } },
        h("svg", { className: "sx-flow-map-svg", viewBox: "0 0 " + layout.width + " " + layout.height, preserveAspectRatio: "none", "aria-hidden": "true" },
          h("defs", null,
            h("marker", { id: "sx-flow-arrow", markerWidth: "8", markerHeight: "8", refX: "7", refY: "3", orient: "auto", markerUnits: "strokeWidth" },
              h("path", { d: "M0,0 L0,6 L7,3 z", fill: "rgba(130,143,255,0.85)" }),
            ),
          ),
          layout.cols.map(function (kind, i) {
            const x = 32 + i * (layout.cols.length > 1 ? (layout.width - 160) / (layout.cols.length - 1) : 0);
            return h("text", { key: kind, x: x, y: 28, className: "sx-flow-col-label" }, kind);
          }),
          edges.map(function (edge) {
            ...
            return h("path", {
              key: edge.id,
              d: d,
              className: "sx-flow-edge sx-flow-edge-configured" + (active ? " sx-flow-edge-active" : ""),
      ...
      h("div", { className: "sx-map-caption" },
        ...
        h(Pill, null, "runtime instances " + layout.nodes.filter(function (n) { return n.origin === "instance"; }).length),
        h(Pill, { band: "green" }, "configured edges " + edges.length),
        h(Pill, null, "observed edges 0 (no honest traversal source yet)"),
      ),
    );
  }
  ...
  function TraceRefList(props) {
    ...
    return h("div", { className: "sx-ref-row" }, items.slice(0, 12).map(function (ref, i) {
      const text = typeof ref === "string" ? ref : ((ref.kind || ref.type || "ref") + ": " + compactId(ref.id || ref.ref || "") + (ref.relation ? " (" + ref.relation + ")" : ""));
  ...
  function TraceDetailRail(props) {
    ...
    return h(Card, { className: "sx-card sx-detail-rail", style: { borderColor: bandColor(flowStatusBand(subject.status)) } }, h(CardContent, { className: "sx-card-content" },
      h("div", { className: "sx-row sx-between" },
        h("div", null, h("div", { className: "sx-title" }, subject.kind || subject.type), h("div", { className: "sx-id" }, subject.id)),
        h(Pill, { band: flowStatusBand(subject.status) }, subject.status || "unknown"),
      ),
      h("div", { className: "sx-meta" },
        h("span", null, "type: ", subject.type),
        h("span", null, "origin: ", subject.origin),
      ),
      ...
      h(Section, { title: "Config refs", subtitle: "sanitized source/kind/version" }, h(TraceRefList, { items: (data.config_refs || []).map(function (c) { return (c.kind || "config") + ": " + (c.ref || "") + (c.version ? " @ " + c.version.slice(0, 18) : ""); }), empty: "Not a configured-topology subject." })),
      h(Section, { title: "Evidence refs" }, h(TraceRefList, { items: (data.evidence_refs || []).map(function (e) { return (e.type || "evidence") + ": " + compactId(e.ref || ""); }), empty: "No evidence refs for this subject." })),
      h("div", { className: "sx-meta" },
  ...
  function FlowDagView() {
    ...
    const graph = buildFlowDagGraph(topo.data, runtime.data);
    const traceState = useTrace(selected);

    return h(Section, { title: "Flow DAG", subtitle: "Configured topology (always shown, including quiet/unobserved blocks) overlaid with current runtime status. Select a node or edge for /trace provenance." },
      (topo.error || runtime.error) ? h(Card, { className: "sx-card sx-error" }, h(CardContent, null, "Flow DAG fetch failed: ", topo.error || runtime.error)) : null,
      h("div", { className: "sx-flow-dag-layout" },
        h(FlowDagCanvas, { graph: graph, selected: selected, onSelect: setSelected }),
        h(TraceDetailRail, { selected: selected, trace: traceState }),
      ),
      h("p", { className: "sx-muted" }, "Configured edges come from /topology; /runtime-status reports zero observed edges in this slice, so no inferred traversal is drawn. Use the Inner-life debug view below for the older /snapshot-shaped projection."),
    );
  }
