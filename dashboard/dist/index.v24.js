(function () {
    ...
    return h("div", { className: "sx-view-nav", role: "tablist", "aria-label": "Sensorium views" },
      views.map(function (v) {
        ...
        return h("button", {
          ...
          title: v.summary || v.label || v.id,
          "aria-label": (v.label || v.id) + (v.countLabel ? ": " + v.countLabel : ""),
          ...
          v.countLabel !== null ? h(Pill, { band: v.band }, v.countLabel || fmt(v.count ?? 0, 0)) : null,
        );
      }),
    );
  }
  ...
  function renderInlineMarkdown(text, keyPrefix) {
    ...
    return parts.map(function (part, index) {
      ...
      if (part.startsWith("`") && part.endsWith("`")) return h("code", { key: key }, part.slice(1, -1));
      if (part.startsWith("**") && part.endsWith("**")) return h("strong", { key: key }, part.slice(2, -2));
      return part;
    });
  }
  ...
  function renderCanonicalMarkdown(markdown) {
    ...
    function isSpecial(line, nextLine) {
      return /^#{1,4}\s/.test(line) || /^>\s?/.test(line) || /^[-*]\s+/.test(line) || /^\d+\.\s+/.test(line) || /^```/.test(line) || (line.includes("|") && nextLine && isTableDivider(nextLine));
    }
    while (index < lines.length) {
      ...
      if (/^```/.test(line)) {
        ...
        index += 1;
        blocks.push(h("pre", { key: "code-" + index, className: "sx-world-model-code" }, h("code", null, code.join("\n"))));
        continue;
      }
      const heading = line.match(/^(#{1,4})\s+(.+)$/);
      if (heading) {
        const level = Math.min(4, heading[1].length + 1);
        blocks.push(h("h" + level, { key: "heading-" + index }, renderInlineMarkdown(heading[2], "heading-" + index)));
        index += 1;
        continue;
      }
      if (line.includes("|") && index + 1 < lines.length && isTableDivider(lines[index + 1])) {
        ...
        blocks.push(h("div", { key: "table-wrap-" + index, className: "sx-world-model-table-wrap" },
          h("table", { className: "sx-world-model-table" },
            h("thead", null, h("tr", null, headers.map(function (cell, cellIndex) { return h("th", { key: "head-" + cellIndex }, renderInlineMarkdown(cell, "head-" + cellIndex)); }))),
            h("tbody", null, rows.map(function (row, rowIndex) { return h("tr", { key: "row-" + rowIndex }, row.map(function (cell, cellIndex) { return h("td", { key: "cell-" + cellIndex }, renderInlineMarkdown(cell, "cell-" + rowIndex + "-" + cellIndex)); })); })),
          ),
        ));
        continue;
      }
      if (/^[-*]\s+/.test(line)) {
        ...
        blocks.push(h("ul", { key: "list-" + index }, items.map(function (item, itemIndex) { return h("li", { key: itemIndex }, renderInlineMarkdown(item, "list-" + index + "-" + itemIndex)); })));
        continue;
      }
      if (/^\d+\.\s+/.test(line)) {
        const items = [];
        while (index < lines.length && /^\d+\.\s+/.test(lines[index])) { items.push(lines[index].replace(/^\d+\.\s+/, "")); index += 1; }
        blocks.push(h("ol", { key: "ordered-" + index }, items.map(function (item, itemIndex) { return h("li", { key: itemIndex }, renderInlineMarkdown(item, "ordered-" + index + "-" + itemIndex)); })));
        continue;
      }
      if (/^>\s?/.test(line)) {
        const quote = [];
        while (index < lines.length && /^>\s?/.test(lines[index])) { quote.push(lines[index].replace(/^>\s?/, "")); index += 1; }
        blocks.push(h("blockquote", { key: "quote-" + index }, renderInlineMarkdown(quote.join(" "), "quote-" + index)));
        continue;
      }
      const paragraph = [line.trim()];
      index += 1;
      while (index < lines.length && lines[index].trim() && !isSpecial(lines[index], lines[index + 1])) { paragraph.push(lines[index].trim()); index += 1; }
      blocks.push(h("p", { key: "paragraph-" + index }, renderInlineMarkdown(paragraph.join(" "), "paragraph-" + index)));
    }
    return blocks;
  }
  ...
  function WorldModelView() {
    ...
    return h(Section, { title: "World Model", subtitle: "Accepted knowledge only. Search, select, and inspect canonical wiki truth with bounded relations and provenance." },
      h("div", { className: "sx-world-model" },
        ...
        search ? h(React.Fragment, null,
          h("div", { className: "sx-world-model-identity" },
            ...
            h(Pill, { band: "green" }, "read-only"),
          ),
          h("details", { className: "sx-verification" },
            h("summary", null, "Verification details"),
            h("div", { className: "sx-verification-body" },
              h("span", null, "build ", corpus.source_build_name || "unknown"),
              h("span", null, "bundle ", corpus.bundle_name || "unknown"),
              h("span", null, "current ", corpus.source_current_digest || "unknown"),
            ),
          ),
        ) : null,
        ...
        pageData ? h(React.Fragment, null,
          h("div", { className: "sx-world-model-identity sx-world-model-selection-state" },
            h(Pill, { band: "green" }, "Accepted knowledge"),
            h(Pill, null, pageData.status || "accepted"),
            h(Pill, null, "privacy " + (pageData.privacy || "private")),
            h(Pill, { band: "green" }, "read-only"),
            h(Pill, null, pageData.page_type || "canonical_page"),
          ),
          h("div", { className: "sx-world-model-layout" },
            h("section", { className: "sx-section sx-world-model-reader" },
              h("div", { className: "sx-section-head" },
                h("h3", null, pageData.title || "Accepted canonical page"),
                h("p", null, "Wiki-owned canonical content. No edits, summaries, or promotion controls."),
              ),
              h("article", { className: "sx-world-model-document" }, renderCanonicalMarkdown(pageData.markdown || "No canonical content available.")),
              h("details", { className: "sx-raw-canonical" },
                h("summary", null, "Raw canonical Markdown"),
                h("p", { className: "sx-muted" }, "Canonical Markdown is lazy-loaded only after selection."),
                h("pre", { className: "sx-world-model-markdown" }, pageData.markdown || "No canonical content available."),
              ),
            ),
            h("aside", { className: "sx-world-model-context", "aria-label": "World Model evidence" },
              ...
              h(Section, { title: "Source and provenance", subtitle: "Opaque verification receipts, never raw source paths." },
  ...
  function DesktopPresentation(props) {
    const p = props.presentation || {};
    const counts = p.counts || {};
    const latest = p.latest || {};
    const artifact = latest.artifact || {};
    const unavailable = p.ok === false || p.posture === "unavailable";
    return h(Section, { title: "Conscious / Desktop presentation", subtitle: unavailable ? "Canonical presentation is unavailable." : "Read-only canonical Conscious doorway state." },
      h("div", { className: "sx-presentation-grid" },
        h(Pill, { band: unavailable ? "red" : (p.posture === "quiet" ? "green" : "yellow"), strong: true }, unavailable ? "unavailable" : (p.posture || "unknown")),
        h(Pill, null, "freshness " + ((p.freshness || {}).state || "unavailable")),
        h("span", { className: "sx-muted" }, "candidates " + (counts.unresolved_candidates ?? 0) + " · apertures " + (counts.open_apertures ?? 0) + " · held " + (counts.held_apertures ?? 0) + " · prepared " + (counts.verified_prepared_reachouts ?? 0) + " · blocked " + (counts.blocked_items ?? 0)),
      ),
      latest.opaque_ref ? h("div", { className: "sx-meta" }, h("span", null, "latest ", latest.opaque_ref), h("span", null, "state ", latest.state || "unknown")) : null,
      artifact.verified ? h("div", { className: "sx-meta" }, h("span", null, "verified artifact hash ", artifact.content_hash), h("span", null, "length ", artifact.content_length)) : null,
    );
  }

  function SensoriumPage() {
    const state = useSnapshot(60 * 1000);
    const presentationState = usePluginJson("/api/plugins/agent-sensorium/presentation", 60 * 1000);
    ...
    const viewCards = [
      { id: "flow_dag", label: "Flow", summary: "Configured topology + live runtime status, with provenance on selection.", countLabel: flowDagNavCount + " nodes", band: "violet" },
      { id: "world_model", label: "Knowledge", summary: "Accepted wiki knowledge, bounded relations, and provenance.", countLabel: "read", band: "green" },
    ].concat(((data && data.views) ? data.views : []).map(function (view) {
      const units = { overview: "pressure", perception: "traces", substrate: "active", actuators: "open" };
      return Object.assign({}, view, { countLabel: fmt(view.count ?? 0, 0) + (units[view.id] ? " " + units[view.id] : "") });
    })).concat([{ id: "inner_life", label: "Debug", summary: "Legacy snapshot-shaped inner-life graph.", countLabel: "debug", band: wrongTurns ? "yellow" : "neutral" }]);
    ...
    const activeCandidates = counts.active_candidates ?? latestCounts.active_candidates ?? 0;
    const nowLine = quiet
      ? "Quiet now. No immediate attention pressure is waiting."
      : plural(activeCandidates, "candidate") + " alive · " + plural(openConscious, "conscious review") + " waiting · " + plural(wrongTurns, "wrong turn") + " observed.";
    ...
    return h("div", { className: "sx-page" },
      h("div", { className: "sx-hero" },
        h("div", null,
          ...
          h("p", { className: "sx-now-line" }, nowLine),
        ),
        h("div", { className: "sx-actions" },
          h(Pill, { band: quiet ? "green" : "yellow", strong: true }, quiet ? "quiet" : plural(liveItems, "live item")),
      ...
      !data ? h(Empty, { text: state.loading ? "Loading Sensorium snapshot…" : "No Sensorium snapshot yet." }) : h(React.Fragment, null,
        h(DesktopPresentation, { presentation: presentationState.data || { ok: false, posture: "unavailable" } }),
        h("div", { className: "sx-metrics sx-priority-metrics" },
          h(MetricCard, { label: "Live pressure", value: liveItems, hint: "candidates + reviews + actions", hot: liveItems > 5 }),
          h(MetricCard, { label: "Conscious reviews", value: openConscious, hint: "waiting now", hot: openConscious > 0 }),
          h(MetricCard, { label: "Active candidates", value: activeCandidates, hint: "attention inbox", hot: activeCandidates > 0 }),
          h(MetricCard, { label: "Foreground-settled", value: liveTurn.foreground_owned_no_residue_count ?? 0, hint: "without duplicate residue" }),
        ),
        ...
        activeView === "overview" ? h(React.Fragment, null,
          h("div", { className: "sx-metrics sx-secondary-metrics" },
            h(MetricCard, { label: "Historical residue", value: residueItems, hint: "held artifacts + historical pointers" }),
        ...
        h("details", { className: "sx-footer sx-verification sx-runtime-details" },
          h("summary", null, "Runtime verification"),
          h("div", { className: "sx-verification-body" },
