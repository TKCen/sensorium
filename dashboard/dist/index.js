(function () {
  ...
  function ArtifactGroupCard(props) {
    const g = props.group;
    const items = g.items || [];
    return React.createElement(Card, { className: "sx-card sx-artifact-group" },
      React.createElement(CardContent, { className: "sx-card-content" },
        React.createElement("div", { className: "sx-row sx-between" },
          React.createElement("div", null,
            React.createElement("div", { className: "sx-title" }, g.title || g.id),
            React.createElement("div", { className: "sx-id" }, g.id),
          ),
          React.createElement(Badge, { variant: "outline", className: "sx-badge" }, (g.count || 0) + " artifacts"),
        ),
        React.createElement("div", { className: "sx-meta" },
          React.createElement("span", null, "type: ", g.group_type || "—"),
          React.createElement("span", null, "kinds: ", countsText(g.kinds)),
          React.createElement("span", null, "states: ", countsText(g.delivery_states)),
          React.createElement("span", null, "updated: ", timeText(g.latest_updated_at)),
        ),
        React.createElement("div", { className: "sx-artifact-mini-list" },
          items.map(function (a) {
            return React.createElement("div", { key: a.id, className: "sx-artifact-mini" },
              React.createElement("span", { className: "sx-artifact-mini-title" }, (a.kind || "artifact") + (a.ref_name ? ": " + a.ref_name : "")),
              React.createElement("span", { className: "sx-id" }, a.id),
              React.createElement("span", { className: "sx-muted" }, a.delivery_state || a.status || "recorded"),
            );
          }),
        ),
      ),
    ...
    return React.createElement("div", { className: "sx-page" },
      ...
      !data ? React.createElement(Empty, { text: state.loading ? "Loading Sensorium snapshot…" : "No Sensorium snapshot yet." }) : React.createElement(React.Fragment, null,
        React.createElement("div", { className: "sx-metrics" },
          ...
          React.createElement(MetricCard, { label: "Artifacts", value: counts.artifact_groups ?? 0, hint: "Groups / held " + (counts.held_artifacts ?? 0) + " / total " + (counts.artifacts ?? 0) }),
        ...
        React.createElement("div", { className: "sx-grid" },
          ...
          React.createElement(Section, { title: "Artifact groups", subtitle: "Grouped by action, thread, candidate, task, or lineage; raw private content stays hidden." },
            data.artifact_groups && data.artifact_groups.length ? data.artifact_groups.map(function (g) { return React.createElement(ArtifactGroupCard, { key: g.id, group: g }); }) : React.createElement(Empty, { text: "No artifacts recorded." }),
          ),
        ),
        React.createElement("div", { className: "sx-grid" },
