(function () {
  ...
  function humanEdgeLabel(kind) {
    const k = String(kind || "relates");
    const labels = {
      emits: "became",
      promotes_to: "raised",
      settles: "settled by",
  ...
  function stageForNode(node) {
    const lane = String((node && node.lane) || "");
    const kind = String((node && node.kind) || "");
    if (lane === "sensors" || lane === "temporal" || kind === "signal" || kind === "event") return "signals";
    if (lane === "candidates" || kind === "candidate") return "meaning";
    if (lane === "reviews" || kind === "action") return "review";
    if (lane === "receipts" || kind === "receipt") return "settled";
    if (lane === "feedback" || kind === "feedback") return "feedback";
    if (lane === "loops" || kind === "loop") return "loop";
    return "meaning";
  }

  function nodeImportance(node) {
    let score = 0;
    const status = String((node && node.status) || "");
    const kind = String((node && node.kind) || "");
    const band = graphBand(node);
    if (band === "red") score += 90;
    if (band === "yellow") score += 60;
    if (status === "candidate" || status === "held" || status === "prepared" || status === "unresolved") score += 55;
    if (kind === "candidate") score += 35;
    if (kind === "signal" || kind === "event") score += 20;
    if (kind === "receipt") score += 10;
    const pressure = Number(node && node.pressure);
    if (Number.isFinite(pressure)) score += Math.min(60, pressure * 80);
    return score;
  }

  function humanGraphLayout(graph) {
    const stageDefs = [
      { id: "signals", label: "Signals", hint: "what touched me" },
      { id: "meaning", label: "Meaning", hint: "what it became" },
      { id: "review", label: "Review", hint: "what asked for action" },
      { id: "settled", label: "Receipts", hint: "what was decided" },
      { id: "feedback", label: "Feedback", hint: "what changed after" },
    ];
    const sourceNodes = (graph.nodes || []).slice();
    const sourceEdges = (graph.edges || []).slice();
    const ranked = sourceNodes.sort(function (a, b) { return nodeImportance(b) - nodeImportance(a); });
    const selected = [];
    const perStage = {};
    ranked.forEach(function (node) {
      const stage = stageForNode(node);
      const cap = stage === "signals" ? 12 : stage === "meaning" ? 16 : 10;
      perStage[stage] = perStage[stage] || 0;
      if (selected.length < 46 && perStage[stage] < cap) {
        perStage[stage] += 1;
        selected.push(node);
      }
    });
    const nodeSet = new Set(selected.map(function (n) { return n.id; }));
    const edges = sourceEdges.filter(function (e) { return nodeSet.has(e.from) && nodeSet.has(e.to); }).slice(0, 80);
    const byStage = {};
    ...
    const width = 1080;
    const top = 88;
    const colGap = width / Math.max(1, stageDefs.length - 1);
    const maxRows = Math.max.apply(null, stageDefs.map(function (s) { return ((byStage[s.id] || []).length || 1); }));
    const height = Math.max(520, top + maxRows * 74 + 84);
    const positioned = [];
    const pos = {};
    stageDefs.forEach(function (stage, stageIndex) {
      const list = byStage[stage.id] || [];
      const x = 48 + stageIndex * colGap;
      list.forEach(function (node, i) {
        const y = top + i * 74 + ((stageIndex % 2) * 22);
        const item = Object.assign({}, node, { stage: stage.id, x: x, y: y });
        positioned.push(item);
        pos[node.id] = { x: x, y: y, node: item };
      });
    });
    const visibleEdges = edges.filter(function (e) { return pos[e.from] && pos[e.to]; });
    return { stageDefs: stageDefs, nodes: positioned, edges: visibleEdges, positions: pos, width: width, height: height, totalNodes: sourceNodes.length, totalEdges: sourceEdges.length };
  }

  function HumanGraphNode(props) {
    const node = props.node;
    const active = props.selected && props.selected.id === node.id;
    const band = node.band || graphBand(node);
    return h("button", {
      type: "button",
      className: "sx-map-node" + (active ? " sx-map-node-active" : ""),
      style: { left: node.x + "px", top: node.y + "px", borderColor: bandColor(band) },
      onClick: function () { props.onSelect(node); },
      title: node.summary || node.label || node.id,
    },
      h("span", { className: "sx-map-node-kind" }, stageForNode(node)),
      h("span", { className: "sx-map-node-label" }, node.label || compactId(node.id)),
      h("span", { className: "sx-map-node-state" }, node.status || node.kind || "observed"),
    );
  }

  function HumanGraphCanvas(props) {
    const layout = humanGraphLayout(props.graph || {});
    const positions = layout.positions;
    return h("div", { className: "sx-map-wrap" },
      h("div", { className: "sx-map-canvas", style: { minWidth: layout.width + "px", height: layout.height + "px" } },
        h("svg", { className: "sx-map-svg", viewBox: "0 0 " + layout.width + " " + layout.height, preserveAspectRatio: "none", "aria-hidden": "true" },
          h("defs", null,
            h("marker", { id: "sx-arrow", markerWidth: "8", markerHeight: "8", refX: "7", refY: "3", orient: "auto", markerUnits: "strokeWidth" },
              h("path", { d: "M0,0 L0,6 L7,3 z", fill: "rgba(148,163,184,0.78)" }),
            ),
          ),
          layout.stageDefs.map(function (stage, i) {
            const x = 48 + i * (layout.width / Math.max(1, layout.stageDefs.length - 1));
            return h("g", { key: stage.id },
              h("line", { x1: x + 80, y1: 56, x2: x + 80, y2: layout.height - 30, stroke: "rgba(255,255,255,0.06)", strokeWidth: "1" }),
              h("text", { x: x, y: 30, className: "sx-map-stage-label" }, stage.label),
              h("text", { x: x, y: 49, className: "sx-map-stage-hint" }, stage.hint),
            );
          }),
          layout.edges.map(function (edge) {
            ...
            return h("g", { key: edge.id || (edge.from + edge.to) },
              h("path", { d: d, className: "sx-map-edge", markerEnd: "url(#sx-arrow)" }),
              Math.abs(y2 - y1) < 38 ? h("text", { x: (x1 + x2) / 2 - 24, y: (y1 + y2) / 2 - 5, className: "sx-map-edge-label" }, humanEdgeLabel(edge.kind)) : null,
            );
          }),
        ),
        layout.nodes.map(function (node) { return h(HumanGraphNode, { key: node.id, node: node, selected: props.selected, onSelect: props.onSelect }); }),
      ),
      h("div", { className: "sx-map-caption" },
        h(Pill, { band: "violet" }, "showing " + layout.nodes.length + " / " + layout.totalNodes + " nodes"),
        h(Pill, null, "visible edges " + layout.edges.length + " / " + layout.totalEdges),
        h("span", null, "Human map first; compact debug projection below."),
      ),
    );
  }
  ...
  function LaneGraphPanel(props) {
    ...
    return h("div", { className: "sx-inner-life-layout" },
      h("div", null,
        h(HumanGraphCanvas, { graph: graph, selected: selected, onSelect: props.onSelect }),
        h("details", { className: "sx-debug-lanes" },
          h("summary", null, "Debug lane projection"),
  ...
  function SensoriumPage() {
    ...
    const viewCards = ((data && data.views) ? data.views : []).concat([{ id: "inner_life", label: "Inner-life", summary: "Living graph of signals, meaning, review, and receipts.", count: ((graphState.data && graphState.data.meta && graphState.data.meta.node_count) || 0) + (perceptionTraces.length || 0), band: wrongTurns ? "yellow" : "violet" }]);
    ...
    return h("div", { className: "sx-page" },
      ...
      !data ? h(Empty, { text: state.loading ? "Loading Sensorium snapshot…" : "No Sensorium snapshot yet." }) : h(React.Fragment, null,
        ...
        activeView === "inner_life" ? h(React.Fragment, null,
          h(Section, { title: "Inner-life living map", subtitle: "Human-first causal graph: what touched me, what it became, what asked for review, and what was settled. Debug lanes are secondary." },
