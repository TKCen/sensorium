...
**Status (Phase 9D):** Feedback settlement and self-loop suppression implemented. `agent_sensorium/gate.py` adds `is_settled_feedback_signal()` and `should_promote_feedback()` helpers that classify ordinary successful internal feedback (`sensorium.action_result`, `sensorium.thread_update`, `sensorium.worker_result` with `source=feedback` and non-failure outcome) as settled proprioception. Settled feedback signals are stored with full causal refs but suppressed at the Signal→Event→Candidate promotion gate — they do not create or update active candidates. Failure/timeout/operator-rejected outcomes, regression/reopen/blocker keywords in summary, explicit `promote_feedback` signal flag, and `promote_all_feedback` config override all bypass suppression. Thread close still marks origin candidate as reviewed. Privacy/surface behavior preserved. 33 new tests covering unit helpers, promotion integration, and end-to-end JSONL behavior including a full smoke sequence proving zero self-loop candidate clutter. All 547 tests pass.
...
2. **Live advisory validation + slow memory reflection substrate:** validate the advisory lane against live accumulated Event substrate, then add a low-frequency Hindsight reflection job that produces compact memory-pressure/reflection material for Subconscious. Hindsight reflection is slow semantic substrate, not an always-on Sensor; it must run infrequently, use bounded prompts/outputs, hash or watermark unchanged reflections, and never act externally.
